...
public class VisTablaPais extends JInternalFrame implements ActionListener  {

	private JTable tblCantones;
	private GestionPais gp;
	
	public VisTablaPais(GestionPais gp){
		this.gp=gp;
		setSize(400,300);        
        getContentPane().setLayout(new BorderLayout());

        JPanel pnlTitulo = new JPanel(new FlowLayout());
        JLabel lblTitulo = new JLabel("Lista Paises");
        pnlTitulo.add(lblTitulo);
		
        tblCantones=new JTable();
        tblCantones.setModel(new ModeloPais());
	...
	public void cargarDatos(){
		tblCantones.setModel(new ModeloPais(gp.getPaises()));
